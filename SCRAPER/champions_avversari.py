...
def main():
    try:
        # URL and table ID for Champions League opponents
        url_champions = "https://fbref.com/it/comp/8/stats/Statistiche-di-Champions-League"
        table_id_champions = "stats_squads_standard_against"

        # Fetch the table
        df_champions = fetch_table(url_champions, table_id_champions)

        # Clean 'Squadra' column
        df_champions['Squadra'] = df_champions['Squadra'].str.replace('vs ', '', regex=False)

        # Save to CSV
        df_champions.to_csv("public/data/champions_avversari.csv", index=False, encoding='utf-8')
        print("champions_avversari.csv has been created successfully.")
