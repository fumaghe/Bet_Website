...
export function DailyMatches({ league, year, highlightLeague }: DailyMatchesProps) {
  ...
  useEffect(() => {
    const loadMatchData = async () => {
      try {
        ...
        console.log(
          `Partite filtrate per la lega "${league}" e l'anno "${year}":`,
          filteredData.length
        );

        if (filteredData.length === 0) {
          throw new Error(
            `Nessuna partita trovata per la lega "${league}" e l'anno "${year}". Verifica il nome della lega e l'anno nel CSV.`
          );
        }
        ...
        // Trova il Sett. più vicino a oggi
  ...
  (matchesToDisplay as MatchDaily[]).forEach(match => {
