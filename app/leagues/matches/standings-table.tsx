...
interface StandingsTableProps {
  league: string; // Add this prop
  limit?: number; // Add this prop
  selectedHomeTeam?: string;
  selectedAwayTeam?: string;
}

export function StandingsTable({ league, limit, selectedHomeTeam, selectedAwayTeam }: StandingsTableProps) {
  ...
  useEffect(() => {
    const fetchStandings = async () => {
      try {
        ...
        setStandings(limit ? leagueStandings.slice(0, limit) : leagueStandings);
